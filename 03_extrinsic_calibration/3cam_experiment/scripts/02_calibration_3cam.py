"""
Robust UR3a 3-camera DH calibration.
...
PATHS = {
    ...
    "results_dir": ROOT / "3cam_experiment" / "results" / "02_calibration_3cam",
}
...
def main() -> None:
    ...
    save_json(PATHS["results_dir"] / "dh_calibrated_3cam.json", dh_out)
    ...
    report = {
        ...
        "outputs": {
            "dh_calibrated": str(PATHS["results_dir"] / "dh_calibrated_3cam.json"),
            "accepted_poses": str(PATHS["results_dir"] / "accepted_poses_3cam.csv"),
            "rejected_poses": str(PATHS["results_dir"] / "rejected_poses_3cam.csv"),
        },
    }
    save_json(PATHS["results_dir"] / "calibration_report_3cam.json", report)

    print("\n================ 3-CAM CALIBRATION SUMMARY ================")
    print(f"Total samples:    {len(all_samples)}")
    print(f"Accepted samples: {len(accepted)}")
    print(f"Rejected samples: {len(rejected)} -> {[int(r['pose_id']) for r in rejected]}")
    print("Nominal all translation RMSE mm:", nominal_all["e_trans_mm"]["rmse"])
    print("Calibrated TEST translation RMSE mm:", result["test_summary"]["e_trans_mm"]["rmse"])
    print("Calibrated TEST rotation RMSE deg:  ", result["test_summary"]["e_rot_deg"]["rmse"])
    print(f"\nSaved calibration results to: {PATHS['results_dir']}")
    print("\nNext run:")
    print("  python 03_validation_3cam.py")
