"""
Estimate / diagnose an improved T_base_cam3 for the 3-camera thesis experiment.

Purpose
-------
This script estimates the camera-3 extrinsic transform using ONLY cam3 images,
robot joint values, nominal/calibrated DH, and the fixed EE->checkerboard transform.

For each successful cam3 image:
    1) Detect checkerboard corners in cam3 image
    2) solvePnP gives T_cam3_CB  (checkerboard/board -> cam3)
    3) FK(q, DH) @ T_EE_CB gives T_base_CB predicted from robot
    4) Therefore each pose estimates:
          T_base_cam3_i = T_base_CB_from_robot @ inv(T_cam3_CB)
    5) Robustly average/refine T_base_cam3 over all poses

Then it compares:
    - existing T_base_cam3 from shared_inputs/tf_base_to_camera.json
    - estimated/refined T_base_cam3 from this script

Outputs
-------
03_extrinsic_calibration/3cam_experiment/results/00_fix_cam3_transform/
    cam3_transform_estimation_report.json
    tf_base_to_camera_cam3_estimated_block.json
    cam3_per_pose_diagnostics.csv

Run
---
python estimate_fix_cam3_base_transform.py
...
PATHS = {
    "cam3_images": ROOT / "images" / "cam3_images",
    "intrinsics_cam3": REPO_ROOT / "02_intrinsic_calibration" / "cam3" / "04_results" / "intrinsics_cam3.json",
    ...
    "results_dir": ROOT / "3cam_experiment" / "results" / "00_fix_cam3_transform",
}
...
# PnP filter. Cam3 single-view PnP should normally be low, often <1-3 px.
MAX_PNP_RMSE_PX = 3.0
...
def image_path_for_pose(pose_id: int) -> Path:
    return PATHS["cam3_images"] / f"img{pose_id}_cam3.png"
...
def build_samples() -> Tuple[List[Cam3Sample], Dict]:
    ensure_dir(PATHS["results_dir"])
    dataset = load_json(PATHS["dataset"])
    intr = load_intrinsics(PATHS["intrinsics_cam3"])
    ...
    for pose in dataset["poses"]:
        ...
        try:
            ...
            samples.append(Cam3Sample(pid, img_path.name, joints_rad, T_cam_CB, T_base_CB_robot, T_base_cam_est_i, rmse))
...
def average_initial_transform(samples: List[Cam3Sample]) -> np.ndarray:
...
def residuals_for_T_base_cam(xi: np.ndarray, T_init: np.ndarray, samples: List[Cam3Sample]) -> np.ndarray:
...
def refine_transform(samples: List[Cam3Sample], T_init: np.ndarray) -> Tuple[np.ndarray, dict]:
...
def transform_errors(T_base_cam: np.ndarray, samples: List[Cam3Sample]) -> List[dict]:
    rows = []
    for s in samples:
...
def main() -> None:
    print("\n================ CAM3 BASE TRANSFORM ESTIMATION ================")
    ...
    if len(usable) < 10:
        ...
        usable = samples_all
    if len(usable) < 6:
        raise RuntimeError("Not enough cam3 PnP samples to estimate a stable transform.")
    ...
    report = {
        "purpose": "Estimate corrected T_base_cam3 from cam3 PnP + robot FK + fixed T_EE_CB, then compare to existing shared tf_base_to_camera cam3.",
        "method": "For each pose: T_base_cam3_i = FK(q,DH) @ T_EE_CB @ inv(T_cam3_CB). Then average/refine T_base_cam3 robustly.",
        "settings": {
            "CAM_ID": CAM_ID,
        ...
        "T_base_cam3_existing_4x4": T_existing.astype(float).tolist(),
        "T_base_cam3_estimated_4x4": T_refined.astype(float).tolist(),
        "per_pose_diagnostics": combined_rows,
    }

    out_dir = PATHS["results_dir"]
    ensure_dir(out_dir)
    save_json(out_dir / "cam3_transform_estimation_report.json", report)
    save_json(out_dir / "tf_base_to_camera_cam3_estimated_block.json", {
        "cam3": {
    ...
    write_csv(out_dir / "cam3_per_pose_diagnostics.csv", combined_rows)

    print("\n================ SUMMARY ================")
    print(f"Usable samples: {len(usable)} / {len(samples_all)}")
    print("\nExisting cam3 vs robot FK:")
    print(json.dumps(existing_summary, indent=2))
    print("\nEstimated cam3 vs robot FK:")
    print(json.dumps(estimated_summary, indent=2))
    print("\nExisting -> estimated difference:")
    print(json.dumps(comparison, indent=2))
    print(f"\nSaved report to: {out_dir / 'cam3_transform_estimation_report.json'}")
    print(f"Saved replacement block to: {out_dir / 'tf_base_to_camera_cam3_estimated_block.json'}")
