"""
UR3a 3-camera checkerboard triangulation + DH calibration + validation pipeline.

What this script does in one run:
1) Detect checkerboard corners in cam1, cam2, cam3 images for each pose.
2) Keep cam1 as the board-origin reference convention.
3) Try all logical 0/90/180/270 corner rotations for cam2 and cam3.
4) Triangulate checkerboard corners in the robot base frame using all 3 cameras.
5) Rigid-align the known checkerboard model to triangulated 3D points -> T_base_CB.
6) Calibrate DH parameters: joint zero offsets + delta a/d, with fixed T_EE_CB.
7) Validate on a held-out test split.
8) Save JSON + CSV diagnostics showing where errors likely come from.

Install:
    pip install numpy opencv-python scipy

Run:
    python run_3cam_triangulate_calibrate_validate.py
...
CAM_INDEX = {"cam1": 1, "cam2": 2, "cam3": 3}
...
PATHS = {
    ...
    "triang_results_dir": ROOT / "3cam_experiment" / "results" / "01_triangulation_3cam",
    "calib_results_dir": ROOT / "3cam_experiment" / "results" / "02_calibration_3cam",
    "valid_results_dir": ROOT / "3cam_experiment" / "results" / "03_validation_3cam",
}
...
def run_triangulation() -> Tuple[dict, dict]:
        ...
        try:
            ...
            for idx2, c2 in enumerate(candidates["cam2"]):
                for idx3, c3 in enumerate(candidates["cam3"]):
                    corners = {"cam1": c1, "cam2": c2, "cam3": c3}
    ...
    triang_json = {
        ...
        "pose_definition": "Main saved measurement is T_base_CB_4x4 estimated from 3-camera triangulation and rigid alignment.",
        "origin_convention": "Cam1 uses the validated board-origin convention. Cam2 and cam3 orderings are selected among 4 logical rotations by minimum 3-view reprojection error plus a small rigid-fit penalty.",
    ...
    write_json(PATHS["triang_results_dir"] / "triangulated_checkerboard_poses_3cam.json", triang_json)
    write_json(PATHS["triang_results_dir"] / "ur3a_joints_data_3cam.json", joints_json)
    write_triangulation_csv(triang_json, PATHS["triang_results_dir"] / "triangulation_diagnostics_3cam.csv")
...
def write_triangulation_csv(triang_json: dict, path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    fields = [
        "pose_id", "success", "reproj_cam1_rmse_px", "reproj_cam2_rmse_px", "reproj_cam3_rmse_px",
...
def split_samples(samples: List[PoseSample]) -> Tuple[List[PoseSample], List[PoseSample], List[PoseSample]]:
    ...
    if len(usable) < 12:
        raise RuntimeError("Not enough usable samples after 3-camera reprojection filtering.")
...
def make_diagnostic_advice(triang_json: dict, nominal_summary: dict, calibrated_summary: dict, calib_result: dict) -> dict:
    ...
    if not advice:
        advice.append("Initial run looks numerically stable. Next improvement step: inspect worst poses, remove only physically justified outliers, then compare 1cam/2cam/3cam under the same train/test split.")
...
def write_validation_csv(rows: List[ErrorRow], path: Path) -> None:
    fields = [
        ...
        "cam1_rmse_px", "cam2_rmse_px", "cam3_rmse_px", "image_triplet"
    ]
    path.parent.mkdir(parents=True, exist_ok=True)
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=fields)
...
def run_calibration_and_validation(triang_json: dict, joints_json: dict) -> Tuple[dict, dict]:
    print("\n================ DH CALIBRATION + VALIDATION: 3 CAMERAS ================")
    ...
    print(f"Loaded matched successful 3-cam samples: {len(samples)}")
    ...
    dh_out = {
        "meta": {
            ...
            "source": "calibrated_from_vision_3cam_triangulation",
        },
    ...
    validation_report = {
        "validation_name": "UR3a_3cam_DH_validation_triangulation",
        "purpose": "Held-out validation comparing 3-camera triangulation-measured checkerboard pose against FK(q,DH) @ T_EE_CB.",
        ...
        "frame_notes": {
            "measurement_pose_in_base": "T_base_CB_meas comes directly from 3-camera triangulation followed by rigid alignment.",
    ...
    write_json(PATHS["calib_results_dir"] / "dh_calibrated_3cam.json", dh_out)
    write_json(PATHS["calib_results_dir"] / "calibration_report_3cam.json", calibration_report)
    write_json(PATHS["valid_results_dir"] / "validation_report_3cam.json", validation_report)
    write_validation_csv(nominal_test_rows, PATHS["valid_results_dir"] / "validation_nominal_test_per_pose_3cam.csv")
    write_validation_csv(calibrated_test_rows, PATHS["valid_results_dir"] / "validation_calibrated_test_per_pose_3cam.csv")

    print(f"\nWrote: {PATHS['calib_results_dir'] / 'dh_calibrated_3cam.json'}")
    print(f"Wrote: {PATHS['calib_results_dir'] / 'calibration_report_3cam.json'}")
    print(f"Wrote: {PATHS['valid_results_dir'] / 'validation_report_3cam.json'}")
...
# =============================================================================
def main() -> None:
    for p in [PATHS["triang_results_dir"], PATHS["calib_results_dir"], PATHS["valid_results_dir"]]:
        ensure_dir(p)

    triang_json, joints_json = run_triangulation()
    calibration_report, validation_report = run_calibration_and_validation(triang_json, joints_json)

    print("\n================ INITIAL 3-CAM RUN SUMMARY ================")
    tri_sum = triang_json["summary"]
    print(f"Triangulation successes: {tri_sum['num_success']}/{tri_sum['num_images_total']}")
    print(f"Nominal test translation RMSE:   {calibration_report['nominal_test_summary']['t_err_mm']['rmse']:.3f} mm")
    print(f"Calibrated test translation RMSE:{calibration_report['best_test_summary']['t_err_mm']['rmse']:.3f} mm")
    print(f"Nominal test rotation RMSE:      {calibration_report['nominal_test_summary']['ang_err_deg']['rmse']:.3f} deg")
    print(f"Calibrated test rotation RMSE:   {calibration_report['best_test_summary']['ang_err_deg']['rmse']:.3f} deg")
    print("\nDiagnostics advice:")
    for item in validation_report["diagnostics_and_next_steps"]["advice"]:
        print("- " + item)
