"""
Robust UR3a 3-camera triangulation + DH calibration + validation.

Purpose:
- Use the already-improved cam3 transform from shared_inputs/tf_base_to_camera.json.
- Triangulate checkerboard pose from cam1+cam2+cam3.
...
Main outputs:
    3cam_experiment/results/04_robust_calibration_3cam/
        robust_calibration_report_3cam.json
        robust_validation_report_3cam.json
        dh_calibrated_3cam_robust.json
        rejected_poses_3cam.csv
        accepted_poses_3cam.csv

Notes:
- This script assumes the 3cam triangulation results already exist from:
...
PATHS = {
    "triangulated": ROOT / "3cam_experiment" / "results" / "01_triangulation_3cam" / "triangulated_checkerboard_poses_3cam.json",
    "joints": ROOT / "3cam_experiment" / "results" / "01_triangulation_3cam" / "ur3a_joints_data_3cam.json",
    ...
    "results_dir": ROOT / "3cam_experiment" / "results" / "04_robust_calibration_3cam",
}
...
MAX_REPROJ_MEAN_PX = 7.0
MAX_REPROJ_MAX_PX = 8.5
MAX_RIGID_FIT_MM = 2.5
...
def build_samples(triang_json: dict, joints_json: dict) -> List[PoseSample]:
    ...
    for p in triang_json["poses"]:
        ...
        out.append(PoseSample(
            pose_id=pid,
            image_triplet=list(jj.get("image_triplet", [p["images"]["cam1"], p["images"]["cam2"], p["images"]["cam3"]])),
...
def main() -> None:
    ...
    nominal_all = evaluate_set(all_samples, dh_nominal, T_EE_CB, None)
    accepted, rejected = filter_samples(all_samples, dh_nominal, T_EE_CB)

    train, test = split_samples(accepted)
    result = calibrate(train, test, dh_nominal, T_EE_CB)

    accepted_rows = []
    for s in accepted:
        nom = pose_error(s, dh_nominal, T_EE_CB, None)
        accepted_rows.append({
            "pose_id": s.pose_id,
            "reproj_mean_rmse_px": s.reproj_mean_rmse_px,
            "reproj_max_rmse_px": s.reproj_max_rmse_px,
            "rigid_fit_rms_mm": s.rigid_fit_rms_mm,
            "nominal_e_trans_mm": nom["e_trans_mm"],
            "nominal_e_rot_deg": nom["e_rot_deg"],
        })

    save_csv(PATHS["results_dir"] / "accepted_poses_3cam.csv", accepted_rows)
    save_csv(PATHS["results_dir"] / "rejected_poses_3cam.csv", rejected)

    dh_out = {
        "meta": {
            "robot": "UR3a",
            "dh_convention": dh_json.get("meta", {}).get("dh_convention", "standard"),
            "units": {"length": "m", "angle": "rad"},
            "source": "robust_calibrated_from_3cam_triangulation_after_outlier_rejection",
        },
        "joints": result["dh_calibrated"],
        "joint_zero_offsets_rad": result["joint_zero_offsets_rad"],
        "joint_zero_offsets_deg": result["joint_zero_offsets_deg"],
    }
    save_json(PATHS["results_dir"] / "dh_calibrated_3cam_robust.json", dh_out)

    report = {
        "settings": {
            "TRAIN_RATIO": TRAIN_RATIO,
            "SEED": SEED,
            "MAX_REPROJ_MEAN_PX": MAX_REPROJ_MEAN_PX,
            "MAX_REPROJ_MAX_PX": MAX_REPROJ_MAX_PX,
            "MAX_RIGID_FIT_MM": MAX_RIGID_FIT_MM,
            "EXCLUDE_POSE_IDS": sorted(list(EXCLUDE_POSE_IDS)),
            "MAX_NOMINAL_TRANS_ERR_MM": MAX_NOMINAL_TRANS_ERR_MM,
            "MAX_NOMINAL_ROT_ERR_DEG": MAX_NOMINAL_ROT_ERR_DEG,
            "JOINT_OFF_BOUND_DEG": JOINT_OFF_BOUND_DEG,
            "DH_BOUND_MM": DH_BOUND_MM,
            "LEVER_ARM_M": LEVER_ARM_M,
            "ROBUST_LOSS": ROBUST_LOSS,
            "ROBUST_F_SCALE": ROBUST_F_SCALE,
            "ENABLE_EXTRINSIC_REFINEMENT": ENABLE_EXTRINSIC_REFINEMENT,
            "CAMERA_IDS_TO_REFINE": CAMERA_IDS_TO_REFINE,
        ...
        "nominal_all_summary_before_filtering": nominal_all,
        "calibration_result": result,
        "interpretation": {
            "why_filtering": "Rejects poses with high triangulation error, bad rigid checkerboard fit, or inconsistent FK-vs-measurement error before optimizing DH.",
            "extrinsic_note": "Camera extrinsic variables are reported as a refinement request. For true camera correction, rerun triangulation after updating tf_base_to_camera.json.",
        },
    }

    validation = {
        "validation_name": "UR3a_3cam_robust_DH_validation",
        "nominal_test": evaluate_set(test, dh_nominal, T_EE_CB, None),
        "robust_calibrated_test": result["test_summary"],
        "train": result["train_summary"],
        "accepted_pose_ids": [s.pose_id for s in accepted],
        "rejected_pose_ids": [int(r["pose_id"]) for r in rejected],
    }

    save_json(PATHS["results_dir"] / "robust_calibration_report_3cam.json", report)
    save_json(PATHS["results_dir"] / "robust_validation_report_3cam.json", validation)

    print("\n================ ROBUST 3-CAM CALIBRATION ================")
    print(f"Total samples:    {len(all_samples)}")
    print(f"Accepted samples: {len(accepted)}")
    print(f"Rejected samples: {len(rejected)} -> {[int(r['pose_id']) for r in rejected]}")
    print("\nNominal all translation RMSE mm:", nominal_all["e_trans_mm"]["rmse"])
    print("Nominal all rotation RMSE deg:  ", nominal_all["e_rot_deg"]["rmse"])
    print("\nRobust calibrated TEST translation RMSE mm:", result["test_summary"]["e_trans_mm"]["rmse"])
    print("Robust calibrated TEST rotation RMSE deg:  ", result["test_summary"]["e_rot_deg"]["rmse"])
    print("\nJoint offsets deg:")
    print(np.array(result["joint_zero_offsets_deg"]))
    print("\nDelta a mm:")
    print(np.array(result["delta_a_mm"]))
    print("Delta d mm:")
    print(np.array(result["delta_d_mm"]))
    print("\nParameter saturation warning:")
    print(json.dumps(result["parameter_saturation_warning"], indent=2))
    print("\nSaved outputs to:")
