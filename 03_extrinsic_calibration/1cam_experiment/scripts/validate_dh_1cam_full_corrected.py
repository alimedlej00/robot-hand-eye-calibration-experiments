...
"""
Corrected full 1-camera validation using the old thesis logic.
Preserves the old equations and measurement chain exactly.
"""
...
def main() -> None:
    ...
    if "transform" in cam_base_json and "T_base_from_camera_4x4" in cam_base_json["transform"]:
        T_base_cam_nominal = np.array(cam_base_json["T_base_to_camera_extrinsics"]["cam1"]["T_4x4"], dtype=float)
    else:
        T_base_cam_nominal = np.array(cam_base_json["T_base_to_camera_extrinsics"]["cam1"]["T_4x4"], dtype=float)
    ...
    report = {
        "validation_name": "UR3a_1cam_DH_validation_compat_oldlogic",
        "purpose": (
        ...
        "models": {
            "nominal": {
                "camera_transform_used": "provided T_base_cam",
